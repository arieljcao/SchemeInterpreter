...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('# argument values does not match with # formal parameters')
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        pair = operands.map(lambda operand: scheme_eval(operand, env))
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.second is nil:
            evaluate = scheme_eval(expressions.first, env, True)
            return evaluate
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN PROBLEM 20
            evaluate = scheme_eval(first, env)
            pairs = rest.map(lambda operand: scheme_eval(operand, env))
            result = scheme_apply(evaluate, pairs, env)
    return result
